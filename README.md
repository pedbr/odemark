...
Open `index.html` through any static file server rooted at this directory.
...
Lantmäteriet data ships, change the attribution string in both `index.html`
files from "förenklad demoterräng …" to the licence line Lantmäteriet
specifies (*© Lantmäteriet* at minimum — confirm exact text with them).
...
- **Zero third-party requests.** Fonts self-hosted, no CDN, no analytics
  ...
  per page, and update the privacy pages which currently say "if enabled".
...
Canonical is ASCII `odemark.se` (all URLs, sitemap, hreflang).
`ödemark.se` (`xn--demark-hva.se`) should 301 to it.
